...
"""
Multilayer Perceptron WITHOUT standardization for calcium imaging data.

This implementation removes all normalization to study how neural networks
naturally adapt to different calcium signal scales through weight learning.
"""
import numpy as np
from sklearn.neural_network import MLPClassifier
from sklearn.model_selection import RandomizedSearchCV
...
class MLPModel:
    ...
    This class removes all data normalization to study how neural networks
    naturally adapt their weights to handle different signal scales.
    """
    ...
    def __init__(self,
                 ...
                 optimize_hyperparams: bool = False):
        """
        Initialize MLP WITHOUT any data preprocessing.
        ...
        # Neural networks will learn to handle different input scales through backpropagation

        logger.info(f"Initialized MLP WITHOUT standardization, hidden layers {hidden_layer_sizes}")
        logger.info(f"  Network will adapt weights to handle natural signal scales")
        logger.info(f"  Raw calcium (~6000), ΔF/F (~0.15), Deconvolved (sparse) maintain distinct characteristics")
    ...
    def _prepare_data(self, X, y=None):
        ...
        logger.info(f"MLP data prepared WITHOUT standardization:")
        logger.info(f"  Shape: {X.shape}")
        logger.info(f"  Mean: {X.mean():.6f}")
        logger.info(f"  Std: {X.std():.6f}")
        logger.info(f"  Min: {X.min():.6f}")
        logger.info(f"  Max: {X.max():.6f}")
        logger.info(f"  Network weights will adapt to these natural input ranges")

        return X, y

    def fit(self, X_train, y_train, X_val=None, y_val=None):
        """
        Train MLP WITHOUT standardization.
        ...
        self
            The trained model
        """
        logger.info("Training MLP WITHOUT standardization")

        # Prepare data WITHOUT any scaling
        X_train, y_train = self._prepare_data(X_train, y_train)

        # REMOVED: self.scaler.fit_transform(X_train)
        # Network will learn appropriate weights for the natural input scales

        # If validation data is provided and early stopping is enabled
        if X_val is not None and y_val is not None and self.early_stopping:
            X_val, y_val = self._prepare_data(X_val, y_val)
            # REMOVED: self.scaler.transform(X_val)
            # Use raw validation data as well

            # Train with built-in early stopping
            self.model.fit(X_train, y_train)

            # Report validation score on raw data
            val_score = self.model.score(X_val, y_val)
            logger.info(f"Validation accuracy (on raw data): {val_score:.4f}")
        else:
            # Train without external validation
            self.model.fit(X_train, y_train)
        ...
        logger.info(f"  Network learned to handle natural signal scale differences")
    ...
    def get_feature_importance(self, window_size: int, n_neurons: int) -> np.ndarray:
        ...
        This reveals how the network naturally weights different features
        when learning from signals at their original scales.
        ...
        logger.info(f"  Weights reflect how network naturally adapts to different signal scales")
        logger.info(f"  Raw calcium, ΔF/F, and Deconvolved signals maintain distinct patterns")
